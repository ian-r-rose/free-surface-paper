...
with dynamic topography calculated as a postprocessing step. 

\hl{address pseudo free surface, free surface, sticky air}

More recently, the nature of surface boundary conditions have been shown to be important for controlling the 
...
In particular, \citet{schmeling2008benchmark} found that the specific properties of the sticky air layer 
...
demonstrating a range of parameters for sticky air which can mitigate some of the difficulties that it 
introduces.
...
Both true free surface and sticky air calculations have been subject to an instability which has been 
variously termed a ``sloshing,'' or ``druknen sailor'' instability \citep{kaus2010stabilization, duretz2011discretization}. 
...
\begin{equation}
\mathrm{Error} = \frac{1}{4\tau}\int_0^{4 \tau} \lVert \zeta_\mathrm{numeric}(L/2, t) - \zeta_\mathrm{analytic}(L/2, t) \rVert_2
\end{equation}
...
Figure~\ref{fig:timestep_convergence} shows the convergence of the nonstandard finite difference scheme 
with timestep.
...
\begin{figure*}
\includegraphics[width=0.9\textwidth]{figures/timestep_convergence_qi.pdf}
\caption{Convergence test with timestep size, quasi-implicit}
\label{fig:timestep_convergence_qi}
\end{figure*}

\begin{figure*}
\includegraphics[width=0.9\textwidth]{figures/tau_sensitivity.pdf}
\caption{Sensitivity to $\tau^*$.}

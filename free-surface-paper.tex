...
\label{fig:stability}
\caption{Left: stability region for the nonstandard finite difference scheme (green). For $\tau^*\le \tau_{\mathrm{max}}$ it is unconditionally stable.  Right: stability region for the quasi-implicit scheme (green).  For $\theta = 1$ it is unconditionally stable. In both cases the stability region for the forward Euler scheme is also shown in blue.}
\end{figure}

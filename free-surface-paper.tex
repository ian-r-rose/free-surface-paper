...
The domain at time $t$ is defined by advecting a reference configuration $\Omega_0$ by a mesh velocity $\mathbf{v}(t)$
\timo{Do we need $\Gamma_N$ here?}
\begin{equation}
 \Omega(t) = \Omega_0 + \int_{t_0}^t \mathbf{v}(t) \text{ d} t.
\end{equation}
The mesh velocity in normal direction is given by the velocity $\mathbf{u}(t)$ on the free surface
\begin{equation}
 \mathbf{v}(t) \cdot n = \mathbf{u}(t) \cdot n \quad \text{on} \quad \Gamma_F
\end{equation}
and set to zero on the rest of the boundary. We defer the discussion of the velocity $\mathbf{v}$ in the interior of the domain
to Section~\ref{sec:remeshing}.
Finally, we denote the displacement at time $t$ by $\zeta = \int_{t_0}^t \mathbf{v}(t) \text{ d} t$, leading to the evolution
equation
\begin{equation}
\frac{d \zeta}{dt} = \mathbf{u \cdot n} \quad \textrm{on  }  \Gamma_F.
...
+ \int_{\Gamma_N(t)} \mathbf{w} \cdot \mathbf{T} \cdot \mathbf{n} = 0 
\label{eq:weak_stokes}
\end{equation}
\timo{change boundary $\Gamma_N$ with new notation above.}
\begin{equation}
\int_{\Omega(t)} q \nabla \cdot \mathbf{u} = 0
\label{eq:weak_incompressible}
\end{equation}
where $\mathbf{w}$ and $q$ are suitably chosen test functions and the integrals over 
$\Omega$ and $\partial \Gamma_N$ are over the volume of the domain and the part of the boundary with stress boundary conditions, respectively.
...
\timo{TODO: update. write as solving on $\Omega(t^n)$ instead of $\Omega(t^{n+1})$}
...
\int_{\Omega^n} \rho  \mathbf{w} \cdot \mathbf{g} + \theta \Delta t \int_{\partial \Omega^n} \rho ( \mathbf{w \cdot g}) (\mathbf{u \cdot n} )
\label{eq:kmm}
\end{equation}
...
\begin{equation}
-\theta \Delta t \int_{\partial \Omega^n} \rho g ( \mathbf{w \cdot n}) (\mathbf{u \cdot n} )
\end{equation}
...
a strong function of the timestep. The remaining error is likely due to nonlinearities, error in the spatial discretization,
or limits in the linear solver tolerance. 
\timo{Could the error be the accuracy of the reference $\zeta$? If you look at 3b, the error also tapers of at 1e-5...}
...
\timo{Change the figure into one with number of unknowns on the x axis and combine into a single figure. Your computed reference value is also not accurate enough. (I can take a look at that)}

\begin{figure*}
\includegraphics[width=0.9\textwidth]{figures/amr.pdf}
\caption{Convergence with resolution for the \citet{crameri2012comparison} Case 2 benchmark, for both uniform (a) and adaptive (b) mesh refinement. The minimum cell size h is normalized by the model depth, and the timestep $\Delta t$ is 500 yr. We compare the maximum topography at 3 Myr with its value at high resolution ($\sim$398 m). The points are labeled with the approximate number of degrees of freedom (DoFs) for that simulation. Both cases converge similarly, but the adaptive mesh refinement simulations require significantly smaller system sizes.}
\label{fig:amr}
\end{figure*}
